...
export default function BehanceIcon(): JSX.Element {
    ...
    try {
      if (!(svgRef.current instanceof HTMLElement))
        throw htmlElementNotFound(
          svgRef.current,
          `validation of Main Reference for ${BehanceIcon.prototype.constructor.name}`
        );
      syncAriaStates(svgRef.current);
      adjustIdentifiers(svgRef.current);
    } catch (e) {
      console.error(
        `Error executing useEffect for ${
          BehanceIcon.prototype.constructor.name
        }:\n${(e as Error).message}`
      );
    }
  }, [svgRef]);
  return (
    <ErrorBoundary FallbackComponent={() => <ErrorIcon fill={true} />}>
      <span id="behanceIconSpan" ref={svgRef}>
        <svg
