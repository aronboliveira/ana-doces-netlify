...
export default function InstIcon(): JSX.Element {
  const svgRef = useRef<nullishAnchor>(null);
  useEffect(() => {
    try {
      if (!(svgRef.current instanceof HTMLElement))
        throw htmlElementNotFound(
          svgRef.current,
          `validation of Instagram SVG Reference`
        );
      syncAriaStates(svgRef.current);
    } catch (e) {
      console.error(
        `Error executing useEffect for InstIcon:\n${(e as Error).message}`
      );
    }
  }, [svgRef]);
  return (
    <a
      className="instIcon socialMediaIcon"
      ref={svgRef}
